...
This document describes how `IvanMurzak/Unity-Mouse-Parallax` signs its
`extensions.unity.mouse.parallax` package so the warning no longer appears in
Unity 6.3+.
...
`extensions.unity.mouse.parallax-`, and uploads the signed `.tgz` as a `signed-upm-package`
workflow artifact.
...
```bash
gh secret set UPM_SERVICE_ACCOUNT_KEY_ID     --repo IvanMurzak/Unity-Mouse-Parallax
gh secret set UPM_SERVICE_ACCOUNT_KEY_SECRET --repo IvanMurzak/Unity-Mouse-Parallax
gh secret set UPM_ORG_ID                     --repo IvanMurzak/Unity-Mouse-Parallax
```
...
If you publish via OpenUPM, OpenUPM's package listing for `extensions.unity.mouse.parallax` likely
...
repository at `data/packages/extensions.unity.mouse.parallax.yml`. Open a PR there changing:
...
1. Go to the release page for the new version and confirm a
   `extensions.unity.mouse.parallax-<version>.tgz` asset is attached alongside the
...
2. Inspect the tarball locally to confirm it contains the signing attestation:
   ...
   curl -fsSL -o package.tgz \
     https://github.com/IvanMurzak/Unity-Mouse-Parallax/releases/download/<version>/extensions.unity.mouse.parallax-<version>.tgz
